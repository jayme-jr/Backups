...
## Bases
munic <- read.csv2("municipios.csv", sep = ";", header = TRUE) %>% 
...
MR1 <- read.csv2("base_MR1.csv", sep = ";", header = TRUE) %>% 
  mutate(`Município` = str_sub(`Município`, start = 8),
         `Macrorregião` = 1) 

MR2 <- read.csv2("base_MR2.csv", sep = ";", header = TRUE) %>% 
  mutate(`Município` = str_sub(`Município`, start = 8),
         `Macrorregião` = 2)

MR3 <- read.csv2("base_MR3.csv", sep = ";", header = TRUE) %>% 
  mutate(`Município` = str_sub(`Município`, start = 8),
         `Macrorregião` = 3)
...
MR4 <- read.csv2("base_MR4.csv", sep = ";", header = TRUE) %>% 
  mutate(`Município` = str_sub(`Município`, start = 8),
         `Macrorregião` = 4)
...
# RESUMO
Com o aumento da violência no trânsito e fatalidades, cresce a necessidade de ações de prevenção de acidentes e cuidados posteriores aos mesmos. Com isso, objetivou-se a fazer uma análise estatística, para determinação de fatores associados às causas de óbitos. Utilizou-se modelagem linear generalizada, com auxílio do software R. Inicialmente foi realizada análise descriva, para então ajuste de modelos e escolha do mais adequado. Com o modelo escolhido foi realizada um reajuste e verificação de predição. O modelo escolhido manteve duas covariáveis que se mostraram significativas, sendo elas relacionadas à frota e urbanização.

# INTRODUÇÃO

A violência no trânsito é um fator bastante preocupante devido ao aumento da população assim como do número de veículos circulantes. As baixas taxas e juros facilitados permitem que mais pessoas tenham acesso à compra de veículos e esses n´úmeros tem como consequência elevados casos de acidentes, muitos deles fatais. Estudos indicam que as maiores taxas de mortalidade estão associadas `a fatores de risco, que podem ser imprudência, falta de educação, insegurança, falta de atendimentos m´edição posterior ao acidente e até mesmo estar relacionada ao gênero do condutor. Identificar as causas de ´óbitos no trânsito permite a execução de ações mitigatórias, tais como planejamentos rodoviários, gestões políticas e outras ações preventivas. 
...
A base de dados principal com a variável resposta utilizada no estudo foi extraída do site **DATASUS** (http://tabnet.datasus.gov.br). As covariáveis população total, população urbana, renda per capta foram extraídas do site Atlas Brasil (http://www.atlasbrasil.org.br/2013/pt/consulta/), já o número de veículos foi extraído do site do IBGE (https://cidades.ibge.gov.br/brasil/pa/pesquisa/22/28120?ano=2017&localidade1=150690). Também foi realizada a subdivisão dos dados em macroregiões, estas foram divididas de acordo com as macroregiões de saúde do estado (4 macroregiões) (Tabela 1). AS variáveis utilizadas na modelagem foram: 
...
6. **pop_urb -** população urbana por município; 
...
Foram testados os modelos da família poissin e Binomial negativa ambos com finção de ligação logarítmica. Na tabela 2 são apresentados os resultados do ajuste para a escolha do modelo.
...
\begin{center}
$$
y_{i}|\underline{x}{i} \sim Binomial \ Negativa (\mu{i},\phi)
$$
...
Foi verificada as medidas de influência, onde foi possível verificar que não há indicativos fortes de outliers ou observações influentes (Figura 7).
...
Também, foi verificado os resíduos quantílicos aleatorizados, que verifica a qualidade do ajuste. Foi possível verificar que o modelo está satisfatóriamente ajustado com porém com as caudas levemente mais pesadas (Figura 8).
...
O gráfico de efeitos, que fornece uma vizualização das mudanças da variável resposta a partir das variações das variáveis explicativas. Na Figura 9, foi observado que o comportamento de todas está dentro das bandas e do esperado para cada uma. Para o log da frota de veículos foi observado efeito positivo e para a população urbana, efeito negativo. O número de óbitos cresce para municípios com maior quantidade de veículos e menos urbanizadas.
...
Após a conclusão da modelagem e ajuste, foram testados alguns perfis fictícios de municípios a fim de ilustrar quais poderiam ser as respostas para cada um deles. Para o perfil 1 foi selecionado frota com valor alto e população intermediária, para o perfil 2 foi selecionado frota com valor intermediário e população baixa e para o perfil 3 foi selecionado frota com valor baixo e população alta. Os valores e as respostas para cada um dos perfis podem ser observados na Tabela 5. Cada uma das respostas significa o valor predito de óbitos no trânsito para caso houvesse algum município que se enquadrasse em cada perfil.
